...
class HomeService {
  ...
  String urlQuery(String? topics, String? sort, String? query,
    ...
    if (topics == null && sort == null && query == null && languages == null) {
      ...
      return "${apiUrl}/books/?page=${page ?? 1}&q=${query ?? ""}";
      ...
      return "${apiUrl}/books/?page=${page ?? 1}&q=${query ?? ""}&topic=${topics ?? ""}";
    } else if (topics == null && languages == null) {
      return "${apiUrl}/books/?page=${page ?? 1}&q=${query ?? ""}&sort=${sort ?? ""}";
      ...
      return "${apiUrl}/books/?page=${page ?? 1}&q=${query ?? ""}&topic=${topics ?? ""}&languages=${languages ?? ""}";
    } else if (topics == null) {
      return "${apiUrl}/books/?page=${page ?? 1}&q=${query ?? ""}&sort=${sort ?? ""}&languages=${languages ?? ""}";
    } else if (languages == null) {
      return "${apiUrl}/books/?page=${page ?? 1}&q=${query ?? ""}&topic=${topics ?? ""}&sort=${sort ?? ""}";
    } else {
      return "${apiUrl}/books/?page=${page ?? 1}&q=${query ?? ""}&topic=${topics ?? ""}&sort=${sort ?? ""}&languages=${languages ?? ""}";
    }
  }
}
